...
- Merges run CI but never publish a binary. Release from a clean, reviewed,
  tested commit using the local signing runbook; never replace published assets.
...
- Release private keys and notarization credentials stay in the local Keychain.
  CI verifies published artifacts, never signs them or claims it built them.
- The Homebrew cask is generated by `scripts/update_homebrew_tap.sh` and must
  pass `brew style --cask`. Never remove quarantine: signed, notarized releases
  must pass Gatekeeper normally.
